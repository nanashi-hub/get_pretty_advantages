...
- 系统 IP 池管理中心：新增系统 IP 池管理页面，支持查看每个 IP 的占用数、剩余容量、到期时间与状态。
- 系统 IP 池：支持新增、编辑、启用、禁用、删除。
- 系统 IP 池：支持批量导入（支持覆盖更新到期时间，适配续费场景）。
- IP 批量运维：IP 列表新增复选框与全选能力，支持批量续期（30/60/90 天）、批量启用/禁用/删除。
- 成员自助新增环境变量：普通成员可为自己直接新增 `ksck` 环境变量（权限边界仅限本人名下）。
- 账号状态自动清理机制：连续 N 天需更换配置的账号自动归档清理（解绑 IP、best-effort 删除青龙变量、从列表隐藏；不影响历史收益与结算）。
- 环境变量列表增强：新增“账号状态”筛选与复选框，支持按筛选结果批量启用/禁用/删除。

### 优化

- IP 占用统计口径统一：IP 使用数仅统计 `status=valid` 的 `ksck` 环境变量；禁用即释放 IP 容量。
- 系统 IP 分配流程优化：`system_random` 模式由后端统一自动分配，不再允许手动选择；当系统 IP 池无可用 IP 时，“系统 IP”选项自动置灰。
- 配置环境列表性能优化：环境变量列表由“按用户串行加载”改为“一次性拉取可管理环境”，页面请求数从数十个降低至 2–3 个。
- 账号状态判定更精细：区分“未统计”与“0 收益”；禁用账号在配置环境列表中显示为“已禁用（灰色）”；仪表盘账号状态提醒仅统计启用账号。

### 机制说明

- IP 自动释放与再分配：`ksck` 禁用即视为释放 IP；`system_random` 模式再启用时自动从可用 IP 池重新分配。
- 自动清理安全策略：连续判定要求“每日有收益记录”，无记录将中断连续计数；清理为逻辑归档，确保收益、结算、审计链路完整。

### 技术细节

- 自动归档任务默认关闭；设置环境变量 `KSCK_AUTO_CLEANUP_DAYS=15` 启用（每日执行一次）。
