...
async def delete_user(
    ...
    from app.models import (
        UserReferral, WalletAccount, WalletLedger,
        UserScriptConfig, UserScriptEnv, EarningRecord
    )

    if db.query(EarningRecord).filter(EarningRecord.user_id == user_id).first():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="该用户存在收益记录，不能删除")

    # 1. 删除钱包交易记录
    try:
        db.query(WalletLedger).filter(WalletLedger.user_id == user_id).delete()
    except Exception:
        pass  # 如果列名不对，跳过
